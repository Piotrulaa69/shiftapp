...
export default function ScheduleScreen() {
  ...
  const navigateWeek = (dir: 1 | -1) => {
    setSelectedDate(fmt(addDays(selDateObj, dir * 7)));
  };

  const navigateDay = (dir: 1 | -1) => {
    setSelectedDate(fmt(addDays(selDateObj, dir)));
  };
  ...
  const monthHolidays = Object.entries(holidays).filter(([d]) => d.startsWith(`${curYear}-${String(curMonth + 1).padStart(2, '0')}`));

  const dayShifts = allShifts.filter((s) => s.day === selectedDate);
  const weekShifts = allShifts.filter((s) => weekDates.includes(s.day));
  ...
  const renderMonthCell = (dateStr: string | null, key: string) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => { setSelectedDate(dateStr); if (calView === 'month') setCalView('day'); }}
        activeOpacity={0.7}
      >
        ...
        {cellShifts.slice(0, 2).map((s) => (
          <View key={s.id} style={[cal.eventBar, { backgroundColor: STATUS_CONFIG[s.status as ShiftStatus]?.color ?? theme.colors.primary }]}>
            <Text style={cal.eventText} numberOfLines={1}>{s.start_time} {s.employee_name?.split(' ')[0]}</Text>
          </View>
        ))}
        {cellShifts.length > 2 && <Text style={cal.moreText}>+{cellShifts.length - 2}</Text>}
      </TouchableOpacity>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.calCard}>
            ...
            <View style={cal.headerRow}>
              {DAY_HEADER.map((h, i) => (
                <Text key={h} style={[cal.headerCell, (i === 5 || i === 6) && { color: theme.colors.error }]}>{h}</Text>
              ))}
            </View>
                ...
                <View key={d} style={[wv.allDayCell, (i === 5 || i === 6) && wv.weekendBg]}>
                  {holidays[d] && (
                    <View style={wv.holidayPill}>
                      <Text style={wv.holidayPillText} numberOfLines={1}>{holidays[d]}</Text>
                    </View>
                  )}
                </View>
              ))}
      ...
      {calView === 'day' && (
        <View style={{ flex: 1 }}>
          {/* Nav bar: Dzisiaj + arrows + date title */}
          <View style={styles.navBar}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Text style={[styles.todayBtnText, selectedDate === today && styles.todayBtnTextActive]}>Dzisiaj</Text>
              </TouchableOpacity>
          ...
          {/* Day header: abbreviation + date circle */}
...
const cal = StyleSheet.create({
  ...
  headerCell: { flex: 1, textAlign: 'center', fontSize: 11, fontWeight: '700', color: theme.colors.textMuted, paddingVertical: 4 },
  gridRow: { flexDirection: 'row' },
  emptyCell: { flex: 1, minHeight: 64 },
  cell: {
    flex: 1,
    minHeight: 64,
    borderWidth: 0.5,
  ...
  dayNumWrap: { width: 22, height: 22, borderRadius: 11, alignItems: 'center', justifyContent: 'center', marginBottom: 2 },
  ...
  holidayText: { fontSize: 9, color: '#16A34A', fontWeight: '700', marginBottom: 1 },
  eventBar: {
    borderRadius: 3,
    paddingHorizontal: 3,
    paddingVertical: 1,
  ...
  eventText: { fontSize: 9, color: '#FFF', fontWeight: '600' },
  moreText: { fontSize: 9, color: theme.colors.textMuted, fontWeight: '600' },
});
